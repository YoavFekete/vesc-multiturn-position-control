...
void commands_process_packet(unsigned char *data, unsigned int len,
	...
	case COMM_SET_POS: {
		int32_t ind = 0;
		mc_interface_set_pid_pos((float)buffer_get_int32(data, &ind) / 1000000.0,0.0,1.0);
		timeout_reset();
	} break;

	case COMM_SET_MULTITURN_POS_FEEDFORWARD: {
        int32_t ind = 0;
		mc_interface_set_pid_pos(unpack_multiturn_pos32( buffer_get_uint32(data, &ind)),(float)buffer_get_int32(data, &ind) / 1000000.0, 1.0);
        timeout_reset();
    } break;
